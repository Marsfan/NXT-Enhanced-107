...
{
  (BMPMAP*)RCXintro_1_BMPMAP_PTR,
  (BMPMAP*)RCXintro_2_BMPMAP_PTR,
  (BMPMAP*)RCXintro_3_BMPMAP_PTR,
  (BMPMAP*)RCXintro_4_BMPMAP_PTR,
  (BMPMAP*)RCXintro_5_BMPMAP_PTR,
  (BMPMAP*)RCXintro_6_BMPMAP_PTR,
  (BMPMAP*)RCXintro_7_BMPMAP_PTR,
  (BMPMAP*)RCXintro_8_BMPMAP_PTR,
  (BMPMAP*)RCXintro_9_BMPMAP_PTR,
  (BMPMAP*)RCXintro_10_BMPMAP_PTR,
  (BMPMAP*)RCXintro_11_BMPMAP_PTR,
  (BMPMAP*)RCXintro_12_BMPMAP_PTR,
  (BMPMAP*)RCXintro_13_BMPMAP_PTR,
  (BMPMAP*)RCXintro_14_BMPMAP_PTR,
  (BMPMAP*)RCXintro_15_BMPMAP_PTR,
  (BMPMAP*)RCXintro_16_BMPMAP_PTR
};
...
UBYTE*    cUiMenuGetIconImage(UBYTE No)
{
  ...
  Image = NULL;
  if (No < (Icons_ICON_PTR->ItemsX * Icons_ICON_PTR->ItemsY))
  {
    Image = (UBYTE*)&Icons_ICON_PTR->Data[No * Icons_ICON_PTR->ItemPixelsX * (Icons_ICON_PTR->ItemPixelsY / 8)];
  }
...
void      cUiUpdateStatus(void)
{
  ...
  if (++VarsUi.UpdateCounter >= RUN_STATUS_CHANGE_TIME)
  {
    ...
    Tmp = (UWORD)((double)Tmp * BATTERY_COUNT_TO_MV);
  ...
  if ((IOMapUi.Flags & UI_ENABLE_STATUS_UPDATE))
  {
    if ((IOMapUi.Flags & UI_UPDATE) || (IOMapUi.Flags & UI_REDRAW_STATUS))
    {
      ...
      if (IOMapUi.BatteryState < Status_ICON_PTR->ItemsX)
      {
...
//
  ...
  switch (IOMapUi.State)
  {
    case INIT_DISPLAY : // Load font and icons
    {
      ...
      pMapDisplay->pFont                              =  (FONT*)Font_ICON_PTR;
      pMapDisplay->pStatusIcons                       =  (ICON*)Status_ICON_PTR;
      pMapDisplay->pStatusText                        =  (UBYTE*)VarsUi.StatusText;
      pMapDisplay->pStepIcons                         =  (ICON*)Step_ICON_PTR;
    ...
    case INIT_INTRO :
    {
      ...
      else
      {
        if (VarsUi.LowBattHasOccured == 0)
        {
          if (VarsUi.LowBatt)
          {
            pMapDisplay->pBitmaps[BITMAP_1] = (BMPMAP*)LowBattery_BMPMAP_PTR;
            pMapDisplay->UpdateMask         =  BITMAP_BIT(BITMAP_1);
    ...
    case BT_ERROR :
    {
      ...
      if (!cUiFeedback((BMPMAP*)Fail_BMPMAP_PTR,Tmp,Tmp + 1,DISPLAY_SHOW_ERROR_TIME))
      {
        IOMapUi.BluetoothState &= ~BT_ERROR_ATTENTION;
        cUiLoadLevel(0,1,1);
        IOMapUi.State           = NEXT_MENU;
