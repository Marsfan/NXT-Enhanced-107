...
const     double SENSORDIVIDER[SENSORS] =
{
...
void      cUiPrintSensorInDisplayBuffer(UBYTE Port,UBYTE Sensor,UBYTE Valid,SLONG Value)
{
  ...
  Tmp = sprintf((char*)VarsUi.DisplayBuffer,(char*)SENSORFORMAT[Sensor - MENU_SENSOR_EMPTY],(double)0);
  if (Valid == TRUE)
  {
    if (Tmp < sprintf((char*)VarsUi.DisplayBuffer,(char*)SENSORFORMAT[Sensor - MENU_SENSOR_EMPTY],(double)Value / SENSORDIVIDER[Sensor - MENU_SENSOR_EMPTY]))
    {
...
UBYTE     cUiVolume(UBYTE Action) // MENU_INIT,MENU_LEFT,MENU_RIGHT,MENU_EXIT
{
  switch (Action)
  {
    case MENU_INIT : // Init time counter and cursor bitmap
    {
      ...
      VarsUi.pTmp       = (UBYTE*)Cursor_BMPMAP_PTR;
  ...
  if (Type < STRINGTYPES)
  {
    switch (VarsUi.GUSState)
    {
      case 0 :  // Init screen
      {
        ...
        pMapDisplay->pBitmaps[BITMAP_1]      = (BMPMAP*)Ok_BMPMAP_PTR;
...
void      cUiDrawPortNo(UBYTE *Bitmap,UBYTE MenuIconNo,UBYTE PortNo)
{
  ...
  Bitmap[6] = Port_ICON_PTR->ItemPixelsX;
  Bitmap[7] = Port_ICON_PTR->ItemPixelsY;
  ...
  switch (Action)
  {
    ...
    case MENU_RUN :
    {
      if ((IOMapUi.Flags & UI_ENABLE_STATUS_UPDATE))
      {
        ...
        {
          VarsUi.RunTimer    = 0;
          if (++VarsUi.RunBitmapPointer >= Running_ICON_PTR->ItemsY )
          {
            VarsUi.RunBitmapPointer      = 0;
          }
    ...
    case MENU_UPDATE :
    {
      pMapDisplay->pMenuIcons[MENUICON_CENTER]  = (UBYTE*)&Running_ICON_PTR->Data[VarsUi.RunBitmapPointer * Running_ICON_PTR->ItemPixelsX * (Running_ICON_PTR->ItemPixelsY / 8)];
      pMapDisplay->UpdateMask                  |= MENUICON_BIT(MENUICON_CENTER);
    }
    break;
  ...
  switch (Action)
  {
    ...
    case MENU_SAVE :                    // Save NXT program
    {
      switch (VarsUi.State)
      {
        ...
        case 2 :
        {
          if (!cUiFeedback((BMPMAP*)Fail_BMPMAP_PTR,TXT_FB_OBP_FILE_EXIST_FAIL,TXT_FB_OBP_OVERWRITE_FAIL,0))
          {
            VarsUi.State          = 0;
          }
        }
        ...
        {
          if (!cUiFeedback((BMPMAP*)Info_BMPMAP_PTR,TXT_FB_OBP_FILE_SAVED_INFO,0,DISPLAY_SHOW_TIME))
          {
            VarsUi.State++;
          }
        }
    ...
    default :                           // Insert selected action/waitfor in program and save if finished
    {
      switch (VarsUi.State)
      {
        ...
        {
          if (!cUiFeedback((BMPMAP*)Fail_BMPMAP_PTR,TXT_FB_OBP_MEMORY_FULL_FAIL,0,DISPLAY_SHOW_ERROR_TIME))
          {
            cUiMenuPrevFile();
            IOMapUi.State = NEXT_MENU;
            VarsUi.State  = 0;
...
UBYTE     cUiFileDelete(UBYTE Action)
{
  if (MENU_INIT == Action)
  {
    switch (VarsUi.State)
    {
      ...
      {
        if (!cUiFeedback((BMPMAP*)Info_BMPMAP_PTR,TXT_FB_FD_FILE_DELETED_INFO,0,DISPLAY_SHOW_TIME))
        {
          IOMapUi.State = EXIT_PRESSED;
          VarsUi.State  = 0;
        }
      ...
      switch (Action)
      {
        ...
        default :
        {
          ...
          if ((Action >= MENU_PORT_1) && (Action <= MENU_PORT_C))
          {
            ...
            pMapDisplay->EraseMask             |= SCREEN_BIT(SCREEN_LARGE);
            pMapDisplay->pBitmaps[BITMAP_1]     = (BMPMAP*)Display_BMPMAP_PTR;
            pMapDisplay->UpdateMask             = BITMAP_BIT(BITMAP_1);
            IOMapUi.Flags                      |=  UI_REDRAW_STATUS;
  ...
  switch (Action)
  {
    case MENU_ON :
    {
      switch (VarsUi.State)
      {
        ...
        {
          if (!cUiFeedback((BMPMAP*)Wait_BMPMAP_PTR,TXT_FB_BT_TURNING_ON_WAIT,0,0))
          {
            VarsUi.State++;
          }
        }
        ...
        {
          if (!cUiFeedback((BMPMAP*)Fail_BMPMAP_PTR,TXT_FB_BT_TURNING_ON_FAIL,0,DISPLAY_SHOW_ERROR_TIME))
          {
            Action = MENU_EXIT;
          }
        }
    ...
    case MENU_OFF :
    {
      switch (VarsUi.State)
      {
        ...
        {
          if (!cUiFeedback((BMPMAP*)Wait_BMPMAP_PTR,TXT_FB_BT_TURNING_OFF_WAIT,0,0))
          {
            VarsUi.State++;
          }
        }
        ...
        default : // Display fail text
        {
          if (!cUiFeedback((BMPMAP*)Fail_BMPMAP_PTR,TXT_FB_BT_TURNING_OFF_FAIL,0,DISPLAY_SHOW_ERROR_TIME))
          {
            Action = MENU_EXIT;
          }
        }
...
UBYTE     cUiBtSearch(UBYTE Action) // Search for devices
{
  if (Action == MENU_INIT) // Init
  {
    switch (VarsUi.State)
    {
      ...
      {
        if (!cUiFeedback((BMPMAP*)Wait_BMPMAP_PTR,TXT_FB_BT_SEARCHING_WAIT,0,0))
        {
      ...
      {
        if (!cUiFeedback((BMPMAP*)Info_BMPMAP_PTR,TXT_FB_BT_SEARCH_ABORTED_INFO,0,DISPLAY_SHOW_TIME))
        {
          VarsUi.State++;
        }
      }
      ...
      default : // Display fail text
      {
        if (!cUiFeedback((BMPMAP*)Fail_BMPMAP_PTR,TXT_FB_BT_SEARCHING_FAIL,0,DISPLAY_SHOW_ERROR_TIME))
        {
          VarsUi.State  = 0;
          IOMapUi.State = EXIT_PRESSED;
        }
...
UBYTE     cUiBtDeviceList(UBYTE Action) // Show devices
{
  switch (Action)
  {
    ...
    case MENU_DELETE : // Remove device from "My contacts"
    {
      switch (VarsUi.State)
      {
        ...
        default : // Display fail text
        {
          if (!cUiFeedback((BMPMAP*)Fail_BMPMAP_PTR,TXT_FB_BT_REMOVE_FAIL,0,DISPLAY_SHOW_ERROR_TIME))
          {
            Action              = MENU_EXIT;
          }
        }
  ...
  if (Action == MENU_DRAW)
  {
    ...
    if (VarsUi.DeviceLeft)
    {
      ...
      pMapDisplay->pMenuIcons[MENUICON_LEFT]    = (UBYTE*)&Devices_ICON_PTR->Data[VarsUi.DeviceType * Devices_ICON_PTR->ItemPixelsX * (Devices_ICON_PTR->ItemPixelsY / 8)];
      pMapDisplay->UpdateMask                  |= MENUICON_BIT(MENUICON_LEFT);
    }
    if (VarsUi.DeviceCenter)
    {
      ...
      cUiBTCommand(UI_BT_GET_DEVICE_TYPE,VarsUi.DevicesKnown,&VarsUi.Tmp,&VarsUi.DeviceType);
      pMapDisplay->pMenuIcons[MENUICON_CENTER]  = (UBYTE*)&Devices_ICON_PTR->Data[VarsUi.DeviceType * Devices_ICON_PTR->ItemPixelsX * (Devices_ICON_PTR->ItemPixelsY / 8)];
      pMapDisplay->UpdateMask                  |= MENUICON_BIT(MENUICON_CENTER);
    }
    if (VarsUi.DeviceRight)
    {
      ...
      cUiBTCommand(UI_BT_GET_DEVICE_TYPE,VarsUi.DevicesKnown,&VarsUi.Tmp,&VarsUi.DeviceType);
      pMapDisplay->pMenuIcons[MENUICON_RIGHT]   = (UBYTE*)&Devices_ICON_PTR->Data[VarsUi.DeviceType * Devices_ICON_PTR->ItemPixelsX * (Devices_ICON_PTR->ItemPixelsY / 8)];
      pMapDisplay->UpdateMask                  |= MENUICON_BIT(MENUICON_RIGHT);
    }
...
UBYTE     cUiBtConnectList(UBYTE Action) // Show connections and maybe disconnect
{
  switch  (Action)
  {
    ...
    case MENU_DISCONNECT : // Disconnect
    {
      switch (VarsUi.State)
      {
        ...
        default : // Display fail text
        {
          if (!cUiFeedback((BMPMAP*)Fail_BMPMAP_PTR,TXT_FB_BT_DISCONNECT_FAIL,0,DISPLAY_SHOW_ERROR_TIME))
          {
            Action          = MENU_EXIT;
          }
        }
  ...
  if (Action == MENU_DRAW)
  {
    ...
    {
      pMapDisplay->pMenuIcons[MENUICON_LEFT]    = (UBYTE*)&Connections_ICON_PTR->Data[VarsUi.Tmp * Connections_ICON_PTR->ItemPixelsX * (Connections_ICON_PTR->ItemPixelsY / 8)];
    }
    ...
    {
      pMapDisplay->pMenuIcons[MENUICON_CENTER]  = (UBYTE*)&Connections_ICON_PTR->Data[VarsUi.Tmp * Connections_ICON_PTR->ItemPixelsX * (Connections_ICON_PTR->ItemPixelsY / 8)];
    }
    ...
    {
      pMapDisplay->pMenuIcons[MENUICON_RIGHT]   = (UBYTE*)&Connections_ICON_PTR->Data[VarsUi.Tmp * Connections_ICON_PTR->ItemPixelsX * (Connections_ICON_PTR->ItemPixelsY / 8)];
    }
    pMapDisplay->EraseMask                     &= ~SCREEN_BIT(SCREEN_LARGE);
    pMapDisplay->EraseMask                     |= MENUICON_BITS;
  ...
  switch  (Action)
  {
    ...
    case MENU_CONNECT : // Insert device
    {
      switch (VarsUi.State)
      {
        ...
        {
          if (!cUiFeedback((BMPMAP*)Wait_BMPMAP_PTR,TXT_FB_BT_CONNECTING_WAIT,0,0))
          {
            if (cUiBTGetDeviceIndex(VarsUi.DevicesKnown,VarsUi.SelectedDevice - 1,&VarsUi.BTIndex))
            {
        ...
        {
          if (!cUiFeedback((BMPMAP*)Fail_BMPMAP_PTR,TXT_FB_BT_CONNECT_BUSY_FAIL,0,DISPLAY_SHOW_ERROR_TIME))
          {
            Action = MENU_EXIT;
          }
        }
        ...
        default : // Display fail text
        {
          if (!cUiFeedback((BMPMAP*)Fail_BMPMAP_PTR,TXT_FB_BT_CONNECTING_FAIL,0,DISPLAY_SHOW_ERROR_TIME))
          {
            Action = MENU_EXIT;
          }
        }
    ...
    case MENU_SEND :
    {
      switch (VarsUi.State)
      {
        ...
        {
          if (!cUiFeedback((BMPMAP*)Fail_BMPMAP_PTR,TXT_FB_BT_SENDING_NO_CONN_FAIL,0,DISPLAY_SHOW_ERROR_TIME))
          {
            Action = MENU_EXIT;
          }
        }
        ...
        {
          if (!cUiFeedback((BMPMAP*)Wait_BMPMAP_PTR,TXT_FB_BT_SENDING_WAIT,0,0))
          {
        ...
        {
          if (!cUiFeedback((BMPMAP*)Fail_BMPMAP_PTR,TXT_FB_BT_SENDING_FAIL,0,DISPLAY_SHOW_ERROR_TIME))
          {
            Action = MENU_EXIT;
          }
        }
  ...
  switch (Action)
  {
    ...
    case MENU_DELETE :
    {
      switch (VarsUi.State)
      {
        ...
        {
          if (!cUiFeedback((BMPMAP*)Info_BMPMAP_PTR,TXT_FB_FD_FILES_INFO,TXT_FB_FD_DELETED_INFO,DISPLAY_SHOW_TIME))
          {
            IOMapUi.State = EXIT_PRESSED;
            VarsUi.State  = 0;
          }
