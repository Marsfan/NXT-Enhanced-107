...
void cCmdCtrl(void)
{
  ...
  switch (VarsCmd.VMState)
  {
    case VM_IDLE:
    {
      ...
      if (IOMapCmd.ActivateFlag == TRUE)
      {
        ...
        else
        {
          ...
          VarsCmd.CmdCtrlOverTimeCnt  = 0;
          VarsCmd.MaxCmdCtrlOverTimeLen = 0;
    ...
    case VM_RUN_SINGLE:
    {
      ...
      if (IOMapCmd.DeactivateFlag == TRUE || pMapButton->State[BTN1] & pMapUi->AbortFlag)
      {
...
#if VM_BENCHMARK
      cmdCtrlTime = dTimerRead() - cmdCtrlTime;
      if (cmdCtrlTime > 1)
      {
        VarsCmd.CmdCtrlOverTimeCnt++;
        if (cmdCtrlTime > VarsCmd.MaxCmdCtrlOverTimeLen)
          VarsCmd.MaxCmdCtrlOverTimeLen = cmdCtrlTime;
      }
#endif
...
NXT_STATUS cCmdInterpUnop1(CODE_WORD * const pCode)
{
  ...
  switch (opCode)
  {
    ...
    case OP_STOPCLUMPIMMED:
    {
        ...
        cCmdReleaseAllMutexes((CLUMP_ID)Arg1);
        cCmdDeQClump(&(VarsCmd.RunQ), (CLUMP_ID)Arg1); //Dequeue the specified clump
        // since we have stopped that clump we should reset its clump rec values.
        CLUMP_REC* pClumpRec = &(VarsCmd.pAllClumps[(CLUMP_ID)Arg1]);
        pClumpRec->PC = 0;
        pClumpRec->CurrFireCount = pClumpRec->InitFireCount;
        pClumpRec->Sleeping = FALSE;
        pClumpRec->WakeTick = 0;
    }
    break;
    
    case OP_STARTCLUMPIMMED:
    {
        cCmdEnQClump(&(VarsCmd.RunQ), (CLUMP_ID)Arg1); //Enqueue the specified clump
        Status = CLUMP_SUSPEND;
    }
    break;
...
//  ULONG TmpULong;
  ...
  if (opCode == OP_NEG || opCode == OP_NOT || opCode == OP_TST || 
      opCode == OP_CMNT || opCode == OP_ABS || opCode == OP_SIGN ||
      (opCode >= OP_ACOS && opCode <= OP_SINH))
  {
  ...
  switch (opCode)
  {
    case OP_MOV:
    {
      UBYTE OptimizeMove = FALSE;
      TypeCode1 = cCmdDSType(INC_ID(Arg1));
      ...
      OptimizeMove = ((cCmdDSType(Arg1) == TC_ARRAY) && ((TypeCode1 == TC_UBYTE) || (TypeCode1 == TC_SBYTE)) &&
                      (cCmdDSType(Arg2) == TC_ARRAY) && ((TypeCode2 == TC_UBYTE) || (TypeCode2 == TC_SBYTE))) ||
                     ((cCmdDSType(Arg1) == TC_ARRAY) && ((TypeCode1 == TC_UWORD) || (TypeCode1 == TC_SWORD)) &&
                      (cCmdDSType(Arg2) == TC_ARRAY) && ((TypeCode2 == TC_UWORD) || (TypeCode2 == TC_SWORD))) ||
                     ((cCmdDSType(Arg1) == TC_ARRAY) && ((TypeCode1 == TC_ULONG) || (TypeCode1 == TC_SLONG)) &&
                      (cCmdDSType(Arg2) == TC_ARRAY) && ((TypeCode2 == TC_ULONG) || (TypeCode2 == TC_SLONG)));
      ElementSize = cCmdSizeOf(TypeCode1);
      if (OptimizeMove)
      {
        ...
        memmove(pArg1, pArg2, (UWORD)Count*ElementSize);
      }
      else
      {
...
/*
  ...
  VarsCmd.SyscallBenchmarks[Arg1][0]++;

  SyscallTime = dTimerRead() - SyscallTime;
  if (SyscallTime > 1)
  {
    VarsCmd.SyscallBenchmarks[Arg1][1]++;
    if (SyscallTime > VarsCmd.SyscallBenchmarks[Arg1][2])
      VarsCmd.SyscallBenchmarks[Arg1][2] = SyscallTime;
  }
*/
#endif
    }
    ...
    case OP_NUMTOSTRING:
    {
      ...
      {
        Count = sprintf(num2StrBuf, "%d", (SLONG)ArgVal2);
      }
      else
      {
        Count = sprintf(num2StrBuf, "%u", ArgVal2);
      }
...
#define RADIANS_PER_DEGREE ((double)0.0174532925)
#define DEGREES_PER_RADIAN ((double)57.2957795)
#define SCALE_FACTOR ((double)100.0)    
    
    case OP_ACOS:
    {
      return (SLONG)(acos((double)Operand/SCALE_FACTOR)*DEGREES_PER_RADIAN);
    }
    
    case OP_ASIN:
    {
      return (SLONG)(asin((double)Operand/SCALE_FACTOR)*DEGREES_PER_RADIAN);
    }

    case OP_ATAN:
    {
      return (SLONG)(atan((double)Operand/SCALE_FACTOR)*DEGREES_PER_RADIAN);
    }

    case OP_CEIL:
    {
      return (SLONG)ceil((double)Operand/SCALE_FACTOR);
    }

    case OP_EXP:
    {
      return (SLONG)exp((double)Operand);
    }

    case OP_FLOOR:
    {
      return (SLONG)floor((double)Operand/SCALE_FACTOR);
    }

    case OP_SQRT:
    {
      return (ULONG)sqrt((double)Operand);
    }

    case OP_TAN:
    {
      return (SLONG)(tan((double)Operand*RADIANS_PER_DEGREE)*SCALE_FACTOR);
    }

    case OP_TANH:
    {
      return (SLONG)(tanh((double)Operand*RADIANS_PER_DEGREE)*SCALE_FACTOR);
    }

    case OP_COS:
    {
      return (SLONG)(cos((double)Operand*RADIANS_PER_DEGREE)*SCALE_FACTOR);
    }

    case OP_COSH:
    {
      return (SLONG)(cosh((double)Operand*RADIANS_PER_DEGREE)*SCALE_FACTOR);
    }

    case OP_SIN:
    {
      return (SLONG)(sin((double)Operand*RADIANS_PER_DEGREE)*SCALE_FACTOR);
    }

    case OP_SINH:
    {
      return (SLONG)(sinh((double)Operand*RADIANS_PER_DEGREE)*SCALE_FACTOR);
    }

    case OP_LOG:
    {
      return (SLONG)(log((double)Operand)*SCALE_FACTOR);
    }

    case OP_LOG10:
    {
      return (SLONG)(log10((double)Operand)*SCALE_FACTOR);
    }
...
NXT_STATUS cCmdInterpBinop(CODE_WORD * const pCode)
{
  ...
  if (opCode == OP_ADD || opCode == OP_SUB || opCode == OP_MUL || 
      ...
      opCode == OP_LSL || opCode == OP_LSR || 
      opCode == OP_ASL || opCode == OP_ASR ||
      opCode == OP_ROTL || opCode == OP_ROTR ||
      opCode == OP_ATAN2 || opCode == OP_FMOD || opCode == OP_POW)
  {
  ...
  switch (opCode)
  {
    ...
    {
      UWORD Count;
      char fmtBuf[80]; // arbitrary limit!!!
      //Assert that the destination is a string (array of bytes)
      NXT_ASSERT(cCmdDSType(Arg1) == TC_ARRAY);
      NXT_ASSERT(cCmdDSType(INC_ID(Arg1)) == TC_UBYTE);

      //Assert that the format is a string (array of bytes)
...
//      return LeftOp ^ RightOp;
    ...
    case OP_ATAN2:
    {
      return (SLONG)atan2(LeftOp, RightOp)*DEGREES_PER_RADIAN;
    }

    case OP_POW:
    {
      return (SLONG)pow(LeftOp, RightOp);
    }
...
//      if (cCmdDSType(Arg4) != TC_ARRAY)
    ...
    case OP_ARROP:
    {
      ...
      {
        ArgVal5 = (UWORD)(ArrayCount3 - ArgVal4);
      }
      ...
      SLONG sval, svaltmp;
      ULONG uval, uvaltmp;
      double avg, delta, sumSqr;
      UBYTE numElements = (MinCount-ArgVal4+1);
          ...
          (Arg1 == OPARR_SUMSQR) || (Arg1 == OPARR_STD)) 
      {
        ...
        if (IS_SIGNED_TYPE(TypeCode3))
        {
          ...
          if (Arg1 == OPARR_MEAN)
            cCmdSetVal(pArg2, TypeCode2, sval/numElements);
        ...
        else
        {
          ...
          if (Arg1 == OPARR_MEAN)
            cCmdSetVal(pArg2, TypeCode2, uval/(MinCount-ArgVal4+1));
          else if (Arg1 != OPARR_STD)
        ...
        if (Arg1 == OPARR_STD) {
          if (IS_SIGNED_TYPE(TypeCode3)) 
            avg = (double)sval/(double)numElements;
          else
            avg = (double)uval/(double)numElements;
          sumSqr = 0;
          for (i = 0; i < MinCount; i++)
          {
            pArg3 = cCmdResolveDataArg(INC_ID(Arg3), ARRAY_ELEM_OFFSET(DVIndex3, ArgVal4 + i), NULL);
            if (IS_SIGNED_TYPE(TypeCode3))
              delta = (double)(SLONG)cCmdGetVal(pArg3, TypeCode3) - avg;
            else
              delta = (double)cCmdGetVal(pArg3, TypeCode3) - avg;
            sumSqr += (delta*delta);
          }
          delta = sqrt(sumSqr / (double)(numElements - 1));
          if (IS_SIGNED_TYPE(TypeCode3))
...
void cCmdWriteBenchmarkFile()
{
  ...
  if (!LOADER_ERR(LStatus))
  {
    ...
    sprintf((char *)Buffer, "CmdCtrl OverTime Count: %d\r\n", VarsCmd.CmdCtrlOverTimeCnt);
    Length = strlen((char *)Buffer);
    LStatus = pMapLoader->pFunc(WRITE, &Handle, Buffer, &Length);

    sprintf((char *)Buffer, "Max CmdCtrl OverTime Length: %d\r\n", VarsCmd.MaxCmdCtrlOverTimeLen);
    Length = strlen((char *)Buffer);
    LStatus = pMapLoader->pFunc(WRITE, &Handle, Buffer, &Length);
    ...
    {
      sprintf((char *)Buffer, "%x\t%d\t%d\t%d\r\n", i, VarsCmd.OpcodeBenchmarks[i][0], VarsCmd.OpcodeBenchmarks[i][1], VarsCmd.OpcodeBenchmarks[i][2]);
      Length = strlen((char *)Buffer);
      LStatus = pMapLoader->pFunc(WRITE, &Handle, Buffer, &Length);
    }
/*
    //syscall benchmarks
    sprintf((char *)Buffer, "Syscall\tCnt\tOver\tMax\r\n");
    Length = strlen((char *)Buffer);
    LStatus = pMapLoader->pFunc(WRITE, &Handle, Buffer, &Length);
    for (i = 0; i < SYSCALL_COUNT; i++)
    {
      sprintf((char *)Buffer, "%d\t%d\t%d\t%d\r\n", i, VarsCmd.SyscallBenchmarks[i][0], VarsCmd.SyscallBenchmarks[i][1], VarsCmd.SyscallBenchmarks[i][2]);
      Length = strlen((char *)Buffer);
      LStatus = pMapLoader->pFunc(WRITE, &Handle, Buffer, &Length);
    }
*/
